    ...
    const checkIfLiked = async () => {
      try {
        if (localStorage.getItem("token")) {
          ...
          const response = await fetch(`http://localhost:3001/art/like/${id}/${localStorage.getItem("user_id")}`, options);
    ...
    const likeArtwork = async () => {
      try {
        if (localStorage.getItem("token")) {
          ...
          const response = await fetch(`http://localhost:3001/art/${id}`, options)
          if (response.status == 200) {
